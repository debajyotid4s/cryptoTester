...
const { requireString, require2x2Matrix, requirePadCharAZ } = require('../utils/validate');
...
router.post('/decrypt', (req, res) => {
  try {
    ...
    const matrix = require2x2Matrix(req.body.params?.matrix, 'matrix');
    const padChar = requirePadCharAZ(req.body.params?.padChar);
    const stripPadding = req.body.params?.stripPadding !== undefined ? req.body.params.stripPadding : false;
    const padAdded = req.body.params?.padAdded || 0;

    const result = decrypt(text, matrix, { padChar, m: 26, stripPadding, padAdded });
